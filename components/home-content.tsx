...
const WhyChooseUsSection = dynamic(
  () => import("@/components/home-sections").then((mod) => ({ default: mod.WhyChooseUsSection })),
  {
    ssr: false,
    loading: () => <div className="py-12 bg-primary/5 rounded-lg" style={{ minHeight: "300px" }}></div>,
  },
)

const HowToUseSection = dynamic(
  () => import("@/components/home-sections").then((mod) => ({ default: mod.HowToUseSection })),
  {
    ssr: false,
    loading: () => <div className="py-12" style={{ minHeight: "300px" }}></div>,
  },
)

const FAQSection = dynamic(() => import("@/components/home-sections").then((mod) => ({ default: mod.FAQSection })), {
  ssr: false,
  loading: () => <div className="py-12 bg-muted/30" style={{ minHeight: "300px" }}></div>,
})
...
export function HomeContent() {
  return (
    <div className="flex flex-col gap-8">
            ...
            <Card key={categoria.id} className="overflow-hidden shadow-md hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="space-y-2">
                  {categoria.calculadoras.slice(0, 3).map((calculadora) => (
                    <Link
                      key={calculadora.id}
      ...
      <Suspense fallback={<div className="py-12 bg-primary/5 rounded-lg" style={{ minHeight: "300px" }}></div>}>
        <WhyChooseUsSection />
      </Suspense>

      <Suspense fallback={<div className="py-12" style={{ minHeight: "300px" }}></div>}>
        <HowToUseSection />
      </Suspense>

      <Suspense fallback={<div className="py-12 bg-muted/30" style={{ minHeight: "300px" }}></div>}>
        <FAQSection />
      </Suspense>
    </div>
  )
